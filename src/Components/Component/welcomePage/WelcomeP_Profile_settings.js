...
export default function WelcomeP_Profile_settings() {
  ...
  const [loading2,            setLoading2]  = useState(false)
    ...
    useEffect(() => {
      if (logged) {
        axios.get(`http://localhost:5000/Customer/${userID}`)
        .then(data => {
  ...
  const Avatar_Form1_submit = async (e) => {
    ...
    try{
      await axios.patch(`http://localhost:5000/Customer/${userID}`, {UserName, Email, defaultPic, Location, Bio, Gender})
  ...
  const Avatar_Form2_submit = async (e) => {
    ...
    try{
      if (OldPassword === userPassword && Password === ConformPassword && Password.length >= 8) {
        axios.patch(`http://localhost:5000/Customer/${userID}`, {Password})
